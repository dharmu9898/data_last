...
<div class="col-xl-12 col-md-12 col-sm-12 col-lg-12">
   ...
   <h2 style="word-spacing:5px;"> <button style="font-size:14px;margin-left:15px;border-radius:6px;" onclick="goBack()"> Back</button>
 
   <span style="margin-left:350px;">{{$gall->slug}} Tour Package</span> </h2>@break;
     @endforeach
...
@foreach ($gallery as $gall)
<p  class="text-center ml-2" align="justify" style="font-size:18px;word-spacing:18px;">  {!! Str::words($gall->Describes,80)!!} </p>@break;
@endforeach
</div>
...
<div style="margin-left:0px;" class="row">
    ...
    <a style="font-size:20px;color:#d34205;" href="{{ route('showcategory',[ 'names' => str_slug($cat->category) ]) }}" class="card-link mt-2">&#9675; &nbsp;{{$cat->category}}</a><br>
    @endforeach 
    ...
    @endforeach
        ...
        @foreach ($gallery as $gall) 
        <center><h3  style="background-color:black;color:white;font-size:25px; text-align: center;text-transform: capitalize;word-spacing:6px;"> Best  Tour  Package in {{$gall->slug}}</h3></center><br>@break;
        @endforeach
...
@foreach ($gallery as $gall) 
<div class="col-md-4">
<input type="hidden" class="form-control"  value="{{$gall->slug}}" name="CatId" id="CatId" placeholder="Search Keyword.." style="font-size:18px;">
...
@include('city')
    ...
    function welcome_fetch_search(){
        manual_filter = $("#manual_filter_tablesm").val();
         cat = $("#CatId").val();
                $.ajax({
                url:"{{ url('showcity/welcome_manualfilterall')}}?manual_filter_tablesm="+manual_filter+"&CatId="+cat,
