...
<div class="col-xl-12 col-md-12 col-sm-12 col-lg-12">
   ...
   <span style="margin-left:350px;">{{$gall->slug2}} Tour Package</span> </h2>@break;
     @endforeach
...
@foreach ($gallery as $gall)
<p  class=" text-center ml-2" align="justify" style="font-size:18px;word-spacing:18px;">  {!! Str::words($gall->desc,80)!!} </p>@break;
@endforeach
</div>
...
<div style="margin-left:0px;" class="row">
    ...
    @endforeach
        ...
        @foreach ($gallery as $gall) 
        <center><h3  style="background-color:black;color:white;font-size:25px; text-align: center;text-transform: capitalize;word-spacing:6px;"> Best  Tour Package in {{$gall->slug2}}</h3></center><br>@break;
        @endforeach
...
<option value="all" class="form-control cnfontsize_2" >Select Your State</option>
...
@foreach ($gallery as $gall) 
<div class="col-md-3">
<input type="hidden" class="form-control"  value="{{$gall->slug2}}" name="CatId" id="CatId" placeholder="Search Keyword.." style="font-size:18px;">
...
@include('category')
    ...
    function welcome_fetch_search(){
        ...
        cat = $("#CatId").val();
                $.ajax({
                url:"{{ url('showcountry/welcome_manualfilterall')}}?manual_filter_tablesm="+manual_filter+"&CatId="+cat,
...
function filter_data(){
  ...
  $.ajax({
          url:"{{ url('showcountry/welcome_manualfilter') }}?filter_state="+manual_filter,
          success:function(data){
            console.log(data);
            $('#myTable tbody').html('');
            $('#myTable tbody').html(data);
...
function filter_city(){
  ...
  $.ajax({
          url:"{{ url('showcountry/welcome_manualfiltercity') }}?filter_city="+manual_filter,
          success:function(data){
            console.log(data);
            $('#myTable tbody').html('');
            $('#myTable tbody').html(data);
