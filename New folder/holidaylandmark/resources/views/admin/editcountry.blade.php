                ...
                <div class="card-body">
                    <div class="col-md-11">
                        <button style="font-size:22px;margin-left:1px;border-radius:9px;" onclick="goBack()">
                            Back</button>
                        <h2 class="header"> Update International Tours </h2>
                        <div style="margin-top: 20px;" class="content">
                            <form method="post" action="{{ route('admin.updatinternational',$data->id) }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-3">
                                        <img src="{{ URL::to('/') }}/public/category/{{ $data->Image }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea rows="10" cols="60" class="form-control" id="desc"
                                        value="{{ old('desc') }}" name="desc" required>{{ $data->desc}}</textarea>
                                    <script src="{{ asset('ckeditor/ckeditor.js') }}" required var
                                        editor=CKEDITOR.replace( 'desc' , { language: 'en' ,
                                    ...
                                    $("form").submit(function(e) {
                                        var messageLength = CKEDITOR.instances['desc'].getData().replace(
                                            /<[^>]*>/gi, '').length;
                                        if (!messageLength) {
