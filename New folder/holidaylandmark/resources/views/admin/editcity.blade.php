...
{{-- messsage pop up open --}}
      ...
      <div class="card">
        ...
        <h2  class="header"> Update Famous State </h2>
          ...
          <form method="post" action="{{ route('admin.updatecity',$data->id) }}" enctype="multipart/form-data">
                  @csrf
    ...
    <div class="form-group col-md-3">
        <img src="{{ URL::to('/') }}/public/category/{{ $data->Image }}" class="rounded-circle" width="60" />
        <input type="hidden" name="hidden_image" value="{{ $data->Image }}" />
             ...
             <textarea rows="10" cols="60" class="form-control" id="Describes"     value="{{ old('Describes') }}" name="Describes" required>{{ $data->Describes}}</textarea>   
             <script src="{{ asset('ckeditor/ckeditor.js') }}" required var editor = CKEDITOR.replace( 'Describes', {
    language: 'en',
...
$("form").submit( function(e) {
            var messageLength = CKEDITOR.instances['Describes'].getData().replace(/<[^>]*>/gi, '').length;
            if( !messageLength ) {
