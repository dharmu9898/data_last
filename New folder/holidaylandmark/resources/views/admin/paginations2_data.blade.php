...
<tr class="text-center">
    <td><img src="{{ URL::to('/') }}/public/category/{{ $gall->Image }}" class="rounded-circle" width="60"
            height="50" /></td>
    <td>{{ $gall->slug }}</td>
    <td> {!! Str::words($gall->desc,5)!!} </td>
    <td width="25%">
        <!-- here is the button action side where you can edit . view and delete the employee record -->
        <form action="{{ route('admin.destroyinternational', $gall->id) }}" method="get">
            <a href="{{ route('admin.showinternational', $gall->id) }}" class="btn btn-sm btn-warning"><span
                    class="fa fa-eye"></span> Show</a>
            <a href="{{ route('admin.editinternational', $gall->id) }}" class="btn btn-sm btn-info"><span
