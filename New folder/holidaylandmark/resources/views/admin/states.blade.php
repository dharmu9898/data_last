...
{{-- messsage pop up open --}}
                  ...
                  @endif 
                    ...
                    <div class="col-12">
                       ...
                       <div class="text-left upgrade-btn">
                          <a href="{{ route('admin.state') }}"  style="font-size:16px;padding:4px;width: 150px;" class="btn btn-primary text-white" > Add Famous state</a>&nbsp;&nbsp;
 ...
 <tbody style="color:black; font:blod; background:#ffff">
  ...
  <td><img src="{{ URL::to('/') }}/public/category/{{ $employee->Image }}" class="rounded-circle" width="60" height="50" /></td>
   ...
   <td> {!! Str::words($employee->Explain,5)!!} </td>
   <td width="25%">
   <!-- here is the button action side where you can edit . view and delete the employee record -->
   <form action="{{ route('admin.destroystate', $employee->id) }}" method="get">
	<a href="{{ route('admin.showstate', $employee->id) }}" class="btn btn-sm btn-warning"><span class="fa fa-eye"></span> Show</a>
	<a href="{{ route('admin.editstate', $employee->id) }}" class="btn btn-sm btn-info"><span class="fa fa-edit"></span> Edit</a>
  @csrf
                    @method('DELETE')
