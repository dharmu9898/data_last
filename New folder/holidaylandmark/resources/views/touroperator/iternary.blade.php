...
.countries,.states,.cities,.trips,.Days{
display: block;
    width: 100%;
    ...
    border-color: none;">
                    <div class="col-md-12 mx-auto">
                        <div class="form-wrapper">
                            ...
                            <form method="post" action="{{route('touroperator.additernary')}}" enctype="multipart/form-data">
...
<select class="trips" name="trips">
         ...
         <strong>Days:</strong>
        			...
        			<select  class="Days" id="Days" name="Days"  value="{{ old('Days') }}"  required>
				<option label="Select a Days"></option>
				<option >day 1</option>
				<option>day 2</option>
				<option >day 3</option>
				<option>day 4</option>
         ...
         <input type="text" name="location"   class="form-control"   value="{{ old('location') }}" placeholder="location" required>
             ...
             <textarea rows="10" cols="60" class="form-control" id="Iternary"     value="{{ old('Iternary') }}" name="Iternary" required>{{ old('Iternary') }}</textarea>   
             <script src="{{ asset('ckeditor/ckeditor.js') }}" required var editor = CKEDITOR.replace( 'Iternary', {
    language: 'en',
...
$("form").submit( function(e) {
            var messageLength = CKEDITOR.instances['Iternary'].getData().replace(/<[^>]*>/gi, '').length;
            if( !messageLength ) {
