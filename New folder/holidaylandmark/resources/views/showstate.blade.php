...
<div class="col-xl-12 col-md-12 col-sm-12 col-lg-12">
   ...
   <h2 style="word-spacing:5px;"><button style="font-size:24px;margin-left:25px;" onclick="goBack()"> Back</button>
 
   <span style="margin-left:350px;">{{$gall->slug1}} Tour Package</span> </h2>@break;
     @endforeach
...
@foreach ($gallery as $gall)
<p  class="text-center ml-2"" align="justify" style="font-size:18px;word-spacing:18px;">  {!! Str::words($gall->Explain,80)!!} </p>@break;
@endforeach
</div>
...
<div style="margin-left:0px;" class="row">
    ...
    @endforeach
        ...
        @foreach ($gallery as $gall) 
        <center><h3  style="background-color:black;color:white;font-size:25px; text-align: center;text-transform: capitalize;word-spacing:6px;"> Best  Tour Package in  {{$gall->slug1}}  </h3></center><br>@break;
        @endforeach
...
@foreach ($gallery as $gall) 
<div class="col-md-3">
<input type="hidden" class="form-control"  value="{{$gall->slug1}}" name="CatId" id="CatId" placeholder="Search Keyword.." style="font-size:18px;">
...
@include('state')
    ...
    function welcome_fetch_search(){
        ...
        cat = $("#CatId").val();
                $.ajax({
                url:"{{ url('showstate/welcome_manualfilterall')}}?manual_filter_tablesm="+manual_filter+"&CatId="+cat,
...
function filter_data(){
  ...
  $.ajax({
          url:"{{ url('showstate/welcome_manualfiltercity') }}?filter_city="+manual_filter,
          success:function(data){
            console.log(data);
            $('#myTable tbody').html('');
            $('#myTable tbody').html(data);
